...
func Visualize() string {
	...
	// To keep the Scopes below the graph and pointing up into the graph we need to
	// group the Nodes in a subgraph cluster, and the scopes into a different subgraph
	// cluster.  THEN we can draw edges between the scopes and nodes.  If we try to
	// cross subgraph cluster borders while still making the subgraphs DOT gets confused.
